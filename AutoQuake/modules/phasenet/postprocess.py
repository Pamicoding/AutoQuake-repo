...
def extract_picks(
    ...
    """
    logging.info(f"we are in extract_picks function in postprocess.py")
    mph = {}
    if config is None: 
        for x in phases:
            mph[x] = 0.3
        ...
        mph["P"] = config.min_p_prob # default=0.3
        mph["S"] = config.min_s_prob # default=0.3
        mph["PS"] = 0.3
        mpd = config.mpd
        pre_idx = int(config.pre_sec / dt) # 100
        post_idx = int(config.post_sec / dt) # 400

    Nb, Nt, Ns, Nc = preds.shape
    logging.info(f"preds.shape: Nb: {Nb}, Nt: {Nt}, Ns: {Ns}, Nc: {Nc}")
    logging.info(f"check file_names")
    if file_names is None:
        file_names = [f"{i:04d}" for i in range(Nb)]
        logging.info(f"first one, {file_names}")
        ...
        logging.info(f"second one, {file_names}")
    else:
        file_names = [x.decode() if isinstance(x, bytes) else x for x in file_names]
        logging.info(f"last one, {file_names}")
    if begin_times is None:
        ...
        logging.info(f"begin_times, {begin_times}")

    picks = []
    for i in range(Nb):
        file_name = file_names[i]
        begin_time = datetime.fromisoformat(begin_times[i])
        logging.info(f"This is index {i}, name: {file_name}, begin_time: {begin_time}")
        for j in range(Ns):
            ...
            for k in range(Nc - 1):  # 0-th channel noise
                ...
                for l, (phase_index, phase_prob) in enumerate(zip(idxs, probs)):
                    logging.info(f"index: {l}, phase_index: {phase_index}, phase_prob: {phase_prob}")
                    pick_time = begin_time + timedelta(seconds=phase_index * dt)
                    logging.info(f"pick_time: {pick_time}")
                    pick = {
                        "file_name": file_name,
                    ...
                    if waveforms is not None:
                        tmp = np.zeros((pre_idx + post_idx, 3)) # 500 x 3 zero array
                        lo = phase_index - pre_idx
                        logging.info(f"lo:{lo}")
                        hi = phase_index + post_idx
                        logging.info(f"hi:{hi}")
                        insert_idx = 0
                        if lo < 0:
                            lo = 0
                            insert_idx = -lo
                        ...
                        #logging.info(f"Filled the {tmp[insert_idx : insert_idx + hi - lo, :]} with {waveforms[i, lo:hi, j, :]}")
                        if use_amplitude:
                            logging.info(f"l: {l}, len(idxs): {len(idxs)}")
                            next_pick = idxs[l + 1] if l < len(idxs) - 1 else (phase_index + post_idx * 3)
                            logging.info(f"next pick: {next_pick}")
                            logging.info(f"amp range: {phase_index}:{min(phase_index + post_idx * 3, next_pick)}")
                            pick["phase_amplitude"] = np.max(
                                amp[phase_index : min(phase_index + post_idx * 3, next_pick)]
                            ).item()  ## peak amplitude
                            #logging.info(f"amp[phase_index : min(phase_index + post_idx * 3, next_pick)] = {amp[phase_index : min(phase_index + post_idx * 3, next_pick)]}")
                            logging.info(f"the final amp being stored is {np.max(amp[phase_index : min(phase_index + post_idx * 3, next_pick)]).item()}")
                    picks.append(pick)
