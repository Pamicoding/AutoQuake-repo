...
def py_func_decorator(output_types=None, output_shapes=None, name=None):
    logging.info(f"py_func_decorator function was called")
    def decorator(func):
...
def dataset_map(iterator, output_types, output_shapes=None, num_parallel_calls=None, name=None, shuffle=False):
    logging.info(f"dataset_map function was called")
    ...
    def index_to_entry(idx):
        '''
        We are now in the index_to_entry function in dataset_map function, this place author use the iterator[idx], 
        since the iterator is an object of DataReader_pred, the bracket notation -> [] will automatically called the 
        magic function start and end with the '__'. In this case, the function __getitem__ was called, as same as iterator.__getitem__(idx).
        '''
        return iterator[idx]
...
class DataReader:
    def __init__(
        ...
        if response_xml is not None:
            ...
            logging.info(f"self.response = obspy.read_inventory(response_xml)")
        else:
            self.response = None
            logging.info(f"self.response = None")
    ...
    def read_mseed(self, fname, response=None, highpass_filter=0.0, sampling_rate=100, return_single_station=True):
        try:
            ...
            if response is not None:
                ...
                logging.info(f"using remove_sensitivity to remove the response.")
        ...
        for trace in stream:
            ...
            logging.info(f"we are executing the demean for {trace}")
            ## highpass filtering > 1Hz
            if highpass_filter > 0.0:
                trace = trace.filter("highpass", freq=highpass_filter)
                logging.info(f"a highpass filter 0.0 was added")
        ...
        nx = len(station_ids)
        #logging.info(f"The nx: {nx}")
        nt = len(stream[0].data)
        #logging.info(f"our sac data looks like {stream[0].data}")
        #logging.info(f"the data length of {stream[0]} is {nt}")
        data = np.zeros([3, nt, nx], dtype=np.float32)
        logging.info(f"Initializing the data with zeros array")
        for i, sta in enumerate(station_keys):
            for j, c in enumerate(sorted(station_ids[sta], key=lambda x: order[x])):
                ...
                if sta[-1] == "N":
                    ...
                    logging.info(f"we are here if the second letter of our seismometer is 'N', for example, HNZ")
        ...
        #logging.info(f"meta looks like:{meta}")
        return meta
    ...
    def read_sac(self, fname):
        ...
        order = ["3", "2", "1", "E", "N", "Z", "O", "F"]
        order = {key: i for i, key in enumerate(order)}
        comp2idx = {"3": 0, "2": 1, "1": 2, "E": 0, "N": 1, "Z": 2, "O": 1, "F": 2}
...
class DataReader_pred(DataReader):
    ...
    def __getitem__(self, i):
        logging.info(f"DataReader.__getitem__() was invoked.")
        base_name = self.data_list[i]
        if self.format == "numpy":
            meta = self.read_numpy(os.path.join(self.data_dir, base_name))
    ...
    def dataset(self, batch_size, num_parallel_calls=2, shuffle=False, drop_remainder=False):
        if self.amplitude:
            ...
            logging.info(f"DataReader_pred.dataset calls dataset_map in it and use 'self' as an argument, which means that the input is a DataReader_pred object!")
        else:
            dataset = dataset_map(
                self,
